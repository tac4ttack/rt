...
/*unsigned int				get_hue(const __local t_scene *scene, t_object __local *obj, t_hit *hit)
{
	unsigned int			hue = obj->color;
	unsigned int			huehue;
	int2					uv;
	if (obj->id == 3)
	    hue = plane_checkerboard(hit->normal, hit->pos, &uv);
	else // TEXTURE SUR SPHERE
	{
		if (obj->pos.x == 0)
		{
			hue = sphere_texture(fast_normalize(obj->pos - hit->pos), scene->texture_earth, 8192, 4096, 0, 0, &uv);
			huehue = sphere_texture(fast_normalize(obj->pos - hit->pos), scene->texture_earth_cloud, 8192, 4096, 1, scene->u_time, &uv);
			hue = blend_add(hue, huehue);
		}
		else
		 	hue = sphere_texture(fast_normalize(obj->pos - hit->pos), scene->texture_moon, 8192, 4096, 0, 0, &uv);
		//hue = sphere_checkerboard(fast_normalize(obj->pos - hit.pos), obj->color);
	}
	return (hue);
}*/
