...
void	init_kernel(t_env *e)
{
	...
	cudaMemcpy(cuda->mem[5], texture[0].pixel_array, sizeof(unsigned int) * texture[0].width * texture[0].height, cudaMemcpyHostToDevice);
	cudaMemcpy(cuda->mem[6], texture[1].pixel_array, sizeof(unsigned int) * texture[1].width * texture[1].height, cudaMemcpyHostToDevice);
	cudaMemcpy(cuda->mem[7], texture[2].pixel_array, sizeof(unsigned int) * texture[2].width * texture[2].height, cudaMemcpyHostToDevice);
