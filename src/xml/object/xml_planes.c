...
/*   Updated: 2018/04/21 23:24:30 by ntoniolo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	xml_plane_data_m(t_env *e, char **att, t_node *plane_node, int *i)
{
	...
	if (ft_strncmp(att[*i], "min=\"", 5) != 0)
		s_error("\x1b[1;31mPlane error, MIN expected in #12\x1b[0m", e);
	else
		xml_data_min_max(e, att, i, &plane_node->min);
	if (ft_strncmp(att[*i], "max=\"", 5) != 0)
		s_error("\x1b[1;31mPlane error, MAX expected in #13\x1b[0m", e);
	else
		xml_data_min_max(e, att, i, &plane_node->max);
	if (ft_strncmp(att[*i], "flags=\"", 7) != 0)
		s_error("\x1b[1;31mPlane error, FLAG expected in #14\x1b[0m", e);
	else
