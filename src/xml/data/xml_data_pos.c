...
/*   Updated: 2018/04/04 15:03:49 by fmessina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
										...
										int *i, t_node *node)
{
	...
	{
		if (attributes[(*i)] == NULL)
			s_error("\x1b[1;31mError reading Plane limit POS value\x1b[0m", e);
		node->limit_pos.x = ft_atof(attributes[(*i)++] + 5);
		node->limit_pos.y = ft_atof(attributes[(*i)++]);
		if (attributes[(*i)][ft_strlen(attributes[*i]) - 1] != '\"')
			s_error("\x1b[1;31mError in Plane limit POS from scene\x1b[0m", e);
		node->limit_pos.z = ft_atof(attributes[(*i)++]);
	}
	else
		s_error("\x1b[1;31mError reading Plane limit POS from scene\x1b[0m", e);
}
