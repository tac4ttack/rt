...
void	texture_load_from_file(t_env *e, char *file, int slot)
{
	...
	if (slot == 0)
	{
		...
		x = 350;
		y = 235;
		printf("\ntest x=%d y=%d | u = %f\n", x, y, e->textures[slot].f_pixels[(x * 3) + (e->textures[slot].w * y)]);
		printf("test x=%d y=%d | v = %f\n", x, y, e->textures[slot].f_pixels[(x * 3) + (e->textures[slot].w * y) + 1]);
		printf("test x=%d y=%d | w = %f\n\n", x, y, e->textures[slot].f_pixels[(x * 3) + (e->textures[slot].w * y) + 2]);
		x = 450;
		y = 75;
